...
export default function ChatBox() {
  ...
  return (
    ...
    //END
    /*<div className={`flex flex-col h-screen p-4 bg-gradient-to-t from-purple-400 to-black ${primary || ""}`}>
        ...
        {message.role === "bot" ? (
          ...
          <span
            className={`p-2 rounded-lg ${
              message.role === "user" ? `${accent || ""} text-white` : `${primary || ""} ${text || ""}`
            }`}
          >
            {message.text}
          </span>
      ...
      <div className="flex items-center mt-4">
        <input
          ...
          className={`flex-1 p-2 text-black rounded-1-md border-b border-1 focus:outline-none focus:border-${accent || ""}`}
        />
        <button
          onClick={handleSendMessage}
          className={`ml-2 p-2 rounded-md bg-${accent || ""} text-white`}
        >
          Send
        </button>
      </div>
    ...
    {messages.map((message, index) => (
      <div
        ...
        className={`mb-4 p-4 rounded-lg ${
          message.role === "user" ? "bg-blue-500 text-white text-right" : "bg-gray-800 text-white text-left"
        }`}
      >
        {message.role === "bot" ? (
          <ReactMarkdown>{message.text}</ReactMarkdown>
        ) : (
          <span className={`p-2 rounded-lg ${message.role === "user" ? "bg-blue-500" : "bg-gray-800"}`}>
            {message.text}
          </span>
        )}
  ...
  <div className="flex items-center mt-4">
    ...
    <button
      onClick={handleSendMessage}
      className={`ml-2 p-3 rounded-md bg-blue-600 hover:bg-blue-700 text-white transition duration-300`}
    >
      Send
    </button>
  </div>
